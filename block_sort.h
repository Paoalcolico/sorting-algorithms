...
struct Range {
    ...
    Range(Iterator start, Iterator end) : start(start), end(end) {
    }
...
RandomAccessIterator FindFirstForward(
...
template <typename RandomAccessIterator, typename T, typename Comparison>
RandomAccessIterator FindLastForward(
    RandomAccessIterator first,
    RandomAccessIterator last,
    const T& value,
    Comparison compare,
...
template <typename RandomAccessIterator, typename T, typename Comparison>
RandomAccessIterator FindFirstBackward(
    RandomAccessIterator first,
    RandomAccessIterator last,
    const T& value,
    Comparison compare,
...
template <typename RandomAccessIterator, typename T, typename Comparison>
RandomAccessIterator FindLastBackward(
    RandomAccessIterator first,
    RandomAccessIterator last,
    const T& value,
    Comparison compare,
...
void MergeInternal(
    ...
    // whenever we find a value to add to the final array, swap it with the value that's already in
    // that spot when this algorithm is finished, 'buffer' will contain its original contents, but
...
class Cache {
   public:
    ...
    Cache(std::size_t size) {
        ...
        // (size + 1)/2 – turns into a full-speed standard merge sort since everything fits into the
        // cache
        cache_size = (size + 1) / 2;
        cache = new (std::nothrow) T[cache_size];
        if (cache) return;

        // sqrt((size + 1)/2) + 1 – this will be the size of the A blocks at the largest level of
        // merges, so a buffer of this size would allow it to skip using internal or in-place merges
        // for anything
        cache_size = std::sqrt(cache_size) + 1;
        cache = new (std::nothrow) T[cache_size];
        if (cache) return;

        // 512 – chosen from careful testing as a good balance between fixed-size memory use and run
        // time
        if (cache_size > 512) {
            cache_size = 512;
            cache = new (std::nothrow) T[cache_size];
        ...
        // 0 – if the system simply cannot allocate any extra memory whatsoever, no memory works
        // just fine
        cache_size = 0;
    }
};
...
// use a small cache to speed up some of the operations
    ...
    while (true) {
        // if every A and B block will fit into the cache, use a special branch specifically for
        // merging with the cache (we use < rather than <= since the block size might be one more
        // than iterator.length())
        if (iterator.length() < cache_size) {
            // if four subarrays fit into the cache, it's faster to merge both pairs of subarrays
            // into the cache, then merge the two merged subarrays from the cache back into the
            // original array
            if ((iterator.length() + 1) * 4 <= cache_size && iterator.length() * 4 <= size) {
                ...
                while (!iterator.finished()) {
                    ...
                    if (compare(*(B1.end - 1), *A1.start)) {
                        // the two ranges are in reverse order, so copy them in reverse order into
                        // the cache
                    ...
                    if (compare(*(B3.end - 1), *A3.start)) {
                        ...
                        // these two ranges weren't already in order, so merge them back into the
                        // array
            ...
            //     6. merge each A block with any B values that follow, using the cache or the
            ...
            // as an optimization, we really only need to pull out the internal buffers once for
            // each level of merges after that we can reuse the same buffers over and over, then
            // redistribute it when we're finished with this level
            ...
            pull[0].count = 0;
            pull[0].range = Range<RandomAccessIterator>(first, first);
            pull[1].count = 0;
            pull[1].range = Range<RandomAccessIterator>(first, first);
            ...
            if (block_size <= cache_size) {
                // if every A block fits into the cache then we won't need the second internal
                // buffer, so we really only need to find 'buffer_size' unique values
                find = buffer_size;
            } else if (find > iterator.length()) {
                // we can't fit both buffers into the same A or B subarray, so find two buffers
                // separately
                find = buffer_size;
                find_separately = true;
            }

            // we need to find either a single contiguous space containing 2√A unique values (which
            // will be split up into two buffers of size √A each), or we need to find one buffer of
            // < 2√A unique values, and a second buffer of √A unique values, OR if we couldn't find
            // that many unique values, we need the largest possible buffer we can get
...
#define PULL(_to)                                                         \
    ...
    pull[pull_index].to = _to
                ...
                if (count >= buffer_size) {
                    // keep track of the range within the array where we'll need to "pull out" these
                    // values to create the internal buffer
                    PULL(A.start);
                    pull_index = 1;

                    if (count == buffer_size + buffer_size) {
                        // we were able to find a single contiguous section containing 2√A unique
                        // values, so this section can be used to contain both of the internal
                        // buffers we'll need
                        buffer1 = Range<RandomAccessIterator>(A.start, A.start + buffer_size);
                        buffer2 =
                            Range<RandomAccessIterator>(A.start + buffer_size, A.start + count);
                        break;
                    } else if (find == buffer_size + buffer_size) {
                        // we found a buffer that contains at least √A unique values, but did not
                        // contain the full 2√A unique values, so we still need to find a second
                        ...
                        // we found a second buffer in an 'A' subarray containing √A unique values,
                        // so we're done!
                ...
                if (count >= buffer_size) {
                    ...
                    if (count == buffer_size + buffer_size) {
                        ...
                        // buffer2 will be pulled out from a 'B' subarray, so if the first buffer
                        // was pulled out from the corresponding 'A' subarray, we need to adjust the
                        // end point for that A subarray so it knows to stop redistributing its
                        // values before reaching buffer2
                        if (pull[0].range.start == A.start) {
                            pull[0].range.end -= pull[1].count;
                        }

                        // we found a second buffer in a 'B' subarray containing √A unique values,
                        // so we're done!
...
#undef PULL
            ...
            for (pull_index = 0; pull_index < 2; ++pull_index) {
                ...
                if (pull[pull_index].to < pull[pull_index].from) {
                    // we're pulling the values out to the left, which means the start of an A
                    // subarray
                    index = pull[pull_index].from;
                    for (count = 1; count < length; ++count) {
                        index = FindFirstBackward(
                            pull[pull_index].to,
                            pull[pull_index].from - (count - 1),
                            *(index - 1),
                            compare,
                            length - count
                        );
            ...
            // now that the two internal buffers have been created, it's time to merge each A+B
            // combination at this level of the merge sort!
            iterator.begin();
            while (!iterator.finished()) {
                Range<RandomAccessIterator> A = iterator.nextRange(first);
                Range<RandomAccessIterator> B = iterator.nextRange(first);
                ...
                if (start == pull[0].range.start) {
                    if (pull[0].from > pull[0].to) {
                        ...
                        // if the internal buffer takes up the entire A or B subarray, then there's
                        // nothing to merge this only happens for very small subarrays, like √4 = 2,
                        // 2 * (2 internal buffers) = 4, which also only happens when cache_size is
                        // small or 0 since it'd otherwise use MergeExternal
                ...
                if (compare(*(B.end - 1), *A.start)) {
                    ...
                    Range<RandomAccessIterator> firstA(
                        A.start,
                        A.start + blockA.length() % block_size
                    );
                    ...
                    // when we leave an A block behind we'll need to merge the previous A block with
                    // any B blocks that follow it, so track that information as well
                    Range<RandomAccessIterator> lastA(firstA);
                    Range<RandomAccessIterator> lastB(first, first);
                    Range<RandomAccessIterator> blockB(
                        B.start,
                        B.start + std::min(block_size, B.length())
                    );
                    blockA.start += firstA.length();
                    RandomAccessIterator indexA = buffer1.start;

                    // if the first unevenly sized A block fits into the cache, copy it there for
                    // when we go to Merge it otherwise, if the second buffer is available, block
                    ...
                    if (blockA.length() > 0) {
                        while (true) {
                            // if there's a previous B block and the first value of the minimum A
                            // block is <= the last value of the previous B block, then drop that
                            // minimum A block behind. or if there are no B blocks left then keep
                            // dropping the remaining A blocks.
                            if ((lastB.length() > 0 && !compare(*(lastB.end - 1), *indexA)) ||
                                blockB.length() == 0) {
                                // figure out where to split the previous B block, and rotate it at
                                // the split
                                RandomAccessIterator B_split =
                                    std::lower_bound(lastB.start, lastB.end, *indexA, compare);
                                std::size_t B_remaining = std::distance(B_split, lastB.end);

                                // swap the minimum A block to the beginning of the rolling A blocks
                                RandomAccessIterator minA = blockA.start;
                                for (RandomAccessIterator findA = minA + block_size;
                                     findA < blockA.end;
                                     findA += block_size) {
                                ...
                                // swap the first item of the previous A block back with its
                                // original value, which is stored in buffer1
                                std::iter_swap(blockA.start, indexA);
                                ++indexA;

                                // locally merge the previous A block with the B values that follow
                                // it if lastA fits into the external cache we'll use that (with
                                // MergeExternal), or if the second internal buffer exists we'll use
                                // that (with MergeInternal), or failing that we'll use a strictly
                                // in-place merge algorithm (MergeInPlace)
                                if (lastA.length() <= cache_size) {
                                    MergeExternal(
                                        lastA.start,
                                        lastA.end,
                                ...
                                if (buffer2.length() > 0 || block_size <= cache_size) {
                                    // copy the previous A block into the cache or buffer2, since
                                    ...
                                    // the area normally taken up by the A block is either the
                                    // contents of buffer2, or data we don't need anymore since we
                                    // memcopied it either way we don't need to retain the order of
                                    // those items, so instead of rotating we can just block swap B
                                    // to where it belongs
                                    std::swap_ranges(
                                        B_split,
                                        B_split + B_remaining,
                                    ...
                                    // we are unable to use the 'buffer2' trick to speed up the
                                    // rotation operation since buffer2 doesn't exist, so perform a
                                    // normal rotation
                                    std::rotate(B_split, blockA.start, blockA.start + block_size);
                                }

                                // update the range for the remaining A blocks, and the range
                                // remaining from the B block after it was split
                                lastA = Range<RandomAccessIterator>(
                                    blockA.start - B_remaining,
                                    blockA.start - B_remaining + block_size
                                );
                                lastB =
                                    Range<RandomAccessIterator>(lastA.end, lastA.end + B_remaining);
                                ...
                                // move the last B block, which is unevenly sized, to before the
                                // remaining A blocks, by using a rotation
                                ...
                                // roll the leftmost A block to the end by swapping it with the next
                                // B block
                                std::swap_ranges(
                                    blockA.start,
                                    blockA.start + block_size,
            ...
            // when we're finished with this merge step we should have the one or two internal
            // buffers left over, where the second buffer is all jumbled up insertion sort the
            // second buffer, then redistribute the buffers back into the array using the opposite
            // process used for creating the buffer

            // while an unstable sort like std::sort could be applied here, in benchmarks it was
            // consistently slightly slower than a simple insertion sort, even for tens of millions
            // of items. this may be because insertion sort is quite fast when the data is already
            // somewhat sorted, like it is here
            InsertionSort(buffer2.start, buffer2.end, compare);

            for (pull_index = 0; pull_index < 2; ++pull_index) {
                std::size_t unique = pull[pull_index].count * 2;
                if (pull[pull_index].from > pull[pull_index].to) {
                    // the values were pulled out to the left, so redistribute them back to the
                    // right
                    ...
                    // the values were pulled out to the right, so redistribute them back to the
                    // left
                    Range<RandomAccessIterator> buffer(
...
int main() {
    ...
    for (int test_case = 0; test_case < sizeof(test_cases) / sizeof(test_cases[0]); test_case++) {
        ...
        for (size_t index = 0; index < total; index++)
            assert(
                !compare(array1[index], array2[index]) && !compare(array2[index], array1[index])
            );
    }
    cout << "passed!" << endl;
#endif
    ...
    for (total = 0; total <= max_size; total += 2048 * 16) {
        ...
        if (time1 >= time2)
            cout << "WikiSort: " << time1 << " seconds, stable_sort: " << time2 << " seconds ("
                 << time2 / time1 * 100.0 << "% as fast)" << endl;
        else
            cout << "WikiSort: " << time1 << " seconds, stable_sort: " << time2 << " seconds ("
                 << time2 / time1 * 100.0 - 100.0 << "% faster)" << endl;

#if PROFILE
        if (compares1 <= compares2)
            cout << "WikiSort: " << compares1 << " compares, stable_sort: " << compares2
                 << " compares (" << compares1 * 100.0 / compares2 << "% as many)" << endl;
        else
            cout << "WikiSort: " << compares1 << " compares, stable_sort: " << compares2
                 << " compares (" << compares1 * 100.0 / compares2 - 100.0 << "% more)" << endl;

        if (assigns1 <= assigns2)
            cout << "WikiSort: " << assigns1 << " assigns, stable_sort: " << assigns2
                 << " assigns (" << assigns1 * 100.0 / assigns2 << "% as many)" << endl;
        else
            cout << "WikiSort: " << assigns1 << " assigns, stable_sort: " << assigns2
                 << " assigns (" << assigns1 * 100.0 / assigns2 - 100.0 << "% more)" << endl;
#endif
    ...
    if (total_time1 >= total_time2)
        cout << "WikiSort: " << total_time1 << " seconds, stable_sort: " << total_time2
             << " seconds (" << total_time2 / total_time1 * 100.0 << "% as fast)" << endl;
    else
        cout << "WikiSort: " << total_time1 << " seconds, stable_sort: " << total_time2
             << " seconds (" << total_time2 / total_time1 * 100.0 - 100.0 << "% faster)" << endl;

#if PROFILE
    if (total_compares1 <= total_compares2)
        cout << "WikiSort: " << total_compares1 << " compares, stable_sort: " << total_compares2
             << " compares (" << total_compares1 * 100.0 / total_compares2 << "% as many)" << endl;
    else
        cout << "WikiSort: " << total_compares1 << " compares, stable_sort: " << total_compares2
             << " compares (" << total_compares1 * 100.0 / total_compares2 - 100.0 << "% more)"
             << endl;

    if (total_assigns1 <= total_assigns2)
        cout << "WikiSort: " << total_assigns1 << " assigns, stable_sort: " << total_assigns2
             << " assigns (" << total_assigns1 * 100.0 / total_assigns2 << "% as many)" << endl;
    else
        cout << "WikiSort: " << total_assigns1 << " assigns, stable_sort: " << total_assigns2
             << " assigns (" << total_assigns1 * 100.0 / total_assigns2 - 100.0 << "% more)"
             << endl;
#endif
